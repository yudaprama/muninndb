...
func TestSetEntityState_PreservesTypeWhenNotProvided(t *testing.T) {
	...
	err := eng.SetEntityState(ctx, vault, "Modbus", "deprecated", "", "")
	require.NoError(t, err)

	rec, err := eng.store.GetEntityRecord(ctx, ws, "modbus")
	require.NoError(t, err)
...
func TestSetEntityState_UpdatesTypeWhenProvided(t *testing.T) {
	...
	err := eng.SetEntityState(ctx, vault, "2014/53/EU", "active", "", "directive")
	require.NoError(t, err)

	rec, err := eng.store.GetEntityRecord(ctx, ws, "2014/53/eu")
	require.NoError(t, err)
	require.NotNil(t, rec)
...
func TestSetEntityState_UpdatesBothStateAndType(t *testing.T) {
	...
	err := eng.SetEntityState(ctx, vault, "Bridge Dongle", "deprecated", "", "module")
	require.NoError(t, err)

	rec, err := eng.store.GetEntityRecord(ctx, ws, "bridge dongle")
	require.NoError(t, err)
	require.NotNil(t, rec)
...
func TestSetEntityState_NotFound(t *testing.T) {
	...
	err := eng.SetEntityState(ctx, vault, "ghost entity", "deprecated", "", "")
...
func TestSetEntityStateBatch_AllSucceed(t *testing.T) {
	...
	errs := eng.SetEntityStateBatch(ctx, vault, ops)
	require.Len(t, errs, 3)
	for i, err := range errs {
		require.NoError(t, err, "op %d should succeed", i)
	}

	rec, _ := eng.store.GetEntityRecord(ctx, ws, "2014/53/eu")
...
func TestSetEntityStateBatch_WithTypeCorrection(t *testing.T) {
	...
	rec, err := eng.store.GetEntityRecord(ctx, ws, "en 62368-1")
	require.NoError(t, err)
